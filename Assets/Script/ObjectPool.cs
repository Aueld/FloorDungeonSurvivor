...
public class ObjectPool : MonoBehaviour
{
    ...
    public List<GameObject> RoundPool { get; private set; }
    ...
    private void Awake()
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            var obj = Instantiate(RoundPrefab, transform);
            obj.SetActive(false);
            RoundPool.Add(obj);
        }
    ...
    public void AllDestroyObject()
    {
        int iCount = transform.childCount;
        for (int i = 0; i < iCount; i++)
        {
            GameObject child = transform.GetChild(i).gameObject;
            if (!child.activeSelf) // 비활성화되어 있으면 전부 파괴
            {
                Destroy(child);
            }
        }
    ...
    public List<GameObject> ReturnActiveAllObj(string name)
    {
