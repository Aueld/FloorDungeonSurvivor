...
public class AlwaysOnTop : MonoBehaviour{
    public static AlwaysOnTop inst;
    #region WIN32API
    ...
    public bool AssignTopmostWindow()
    {
#if !UNITY_EDITOR
        System.IntPtr hWnd = FindWindow((string)null, "LoveInLogin");

        RECT rect = new RECT();
        GetWindowRect(new HandleRef(this, hWnd), out rect);

        if (DataController.duSettingData[11].value < 0.5f)
        { return SetWindowPos(hWnd, HWND_TOPMOST, rect.X, rect.Y, rect.Width, rect.Height, SWP_SHOWWINDOW); }
        else
        { return SetWindowPos(hWnd, HWND_NOT_TOPMOST, rect.X, rect.Y, rect.Width, rect.Height, SWP_SHOWWINDOW); }
#endif
        return true;
    }
